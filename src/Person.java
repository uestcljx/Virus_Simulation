...
public class Person extends Point {
    ...
    public interface State {
        ...
        int DEATH = -2;//病死者
        int CURED = -1; //治愈数量用于计算治愈出院后归还床位数量，该状态是否存续待定
    }
    ...
    public void update() {
        ...
        if ((state == State.CONFIRMED || state == State.FREEZE) && dieMoment == 0) {
            ...
            double fatality = (state == State.CONFIRMED)?Constants.MORTALITYNoBed:Constants.MORTALITY;
            if (1 <= destiny && destiny <= (int) (fatality * 10000)) {
                //如果幸运数字落在死亡区间
                ...
                cureMoment = confirmedTime + cureTime;//发病后确定治愈时刻
            }
        }
        ...
        if (state == State.CONFIRMED || state == State.FREEZE) {
            //处理病死者
            ...
            //处理治愈者
            else if(MyPanel.worldTime >= cureMoment && cureMoment > 0){
                state = State.CURED;//患者治愈
                Hospital.getInstance().returnBed(useBed);//归还床位
                int resetX = (int)MathUtil.stdGaussian(100, city.getCenterX());
                int resetY = (int)MathUtil.stdGaussian(100, city.getCenterY());
                if (resetX > 700) {
                    resetX = 700;
                }
                setX(resetX);
        ...
        for (Person person : people) {
            if (person.getState() == State.NORMAL || person.getState() == State.CURED || person.getState() == State.DEATH) {
                continue;
            }
            float random = new Random().nextFloat();
            float infectedProb = (state == State.NORMAL)?Constants.BROAD_RATE:(Constants.BROAD_RATE-Constants.IMMUNITY);
            if (random < infectedProb && distance(person) < SAFE_DIST) {
                this.beInfected();
                break;
            }
        }
